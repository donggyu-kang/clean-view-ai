...
const STEP_TEMPLATES = [
  { label: '요청 수신',   sub: 'Spring Boot',       icon: 'inbox'  },
  { label: '기억 검색',   sub: 'FastAPI · pgvector', icon: 'search' },
  { label: '답변 생성',   sub: 'GPT-4o',             icon: 'zap'    },
]

function buildSteps(traceData) {
  if (!traceData) return null
  return [
    { ...STEP_TEMPLATES[0], ms: null },
    { ...STEP_TEMPLATES[1], ms: traceData.retrieval_duration_ms ?? null },
    { ...STEP_TEMPLATES[2], ms: traceData.generation_duration_ms ?? null },
  ]
}

export function TraceModal({ open, onClose, traceInfo }) {
  ...
  const traceData   = traceInfo?.trace ?? null
  const references  = traceInfo?.references ?? []
  const steps       = buildSteps(traceData)

  const totalMs = traceData
    ? (traceData.retrieval_duration_ms ?? 0) + (traceData.generation_duration_ms ?? 0)
    : null

  const memoryCount     = traceData?.retrieval_scores?.length ?? references.length
  const retrievalMs     = traceData?.retrieval_duration_ms ?? null
    ...
    <div style={{
      ...
      <div onClick={e => e.stopPropagation()} style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div>
              ...
              <div style={{ fontSize: 12, color: T.textMid }}>
                {totalMs != null ? `총 ${totalMs}ms 걸려 답변을 만들었어요` : '처리 시간을 가져오는 중...'}
              </div>
            </div>
        ...
        <div style={{ padding: '24px 28px' }}>
          {(steps ?? STEP_TEMPLATES.map((s, i) => ({ ...s, ms: [38, 124, 921][i] }))).map((step, i, arr) => {
            const usedMs  = step.ms ?? 0
            const sumMs   = arr.reduce((s, p) => s + (p.ms ?? 0), 0) || 1
            const off     = (arr.slice(0, i).reduce((s, p) => s + (p.ms ?? 0), 0) / sumMs) * 100
            const w       = (usedMs / sumMs) * 100
            return (
              <div key={step.label} style={{ marginBottom: i < arr.length - 1 ? 18 : 0 }}>
          ...
          <div style={{ padding: '4px 28px 20px' }}>
            <div style={{
              ...
              <div style={{ padding: '10px 14px', display: 'flex', flexDirection: 'column', gap: 7 }}>
                {[
                  ['참고한 기억 수', memoryCount > 0 ? `${memoryCount}개` : '—'],
                  ['검색 소요 시간', retrievalMs != null ? `${retrievalMs}ms` : '—'],
                  ...(traceData.retrieval_scores?.length > 0
                    ? [['유사도 스코어', traceData.retrieval_scores.map(s => `${Math.round(s * 100)}%`).join(', ')]]
                    : []),
                ].map(([key, val]) => (
        ...
        <div style={{ padding: '16px 28px 24px', borderTop: `1px solid ${T.border}`, display: 'flex' }}>
          {[
            { label: '총 소요 시간',  val: totalMs != null ? `${totalMs}ms` : '—' },
            { label: '참고한 기억',   val: memoryCount > 0 ? `${memoryCount}개` : '—' },
            { label: '검색 소요',     val: retrievalMs != null ? `${retrievalMs}ms` : '—' },
            { label: '답변 생성',     val: traceData?.generation_duration_ms != null ? `${traceData.generation_duration_ms}ms` : '—' },
          ].map((s, i) => (
